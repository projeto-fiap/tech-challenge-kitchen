...
public class KitchenFlow {
    ...
    public void createKitchenOrder() {
        pedidoId = 1L;
        kitchenDTO = kitchenService.create(pedidoId).orElseThrow(() -> new IllegalStateException("Pedido não foi criado corretamente"));
    }
    ...
    public void deveMoverOPedidoParaPRODUCTION() {
        kitchenDTO = kitchenService.setInProduction(pedidoId).orElseThrow(() -> new IllegalStateException("Pedido não foi movido para PRODUCTION"));
        assertEquals("IN_PRODUCTION", kitchenDTO.getStatus().toString(), "O pedido não foi movido corretamente para Production!");
    }
}
